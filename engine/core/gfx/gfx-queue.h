...
private:
    // 现有的成员变量...
    VkBuffer _uniformBuffers;
    VkDeviceMemory _uniformBuffersMemory;
    void *_uniformBuffersMapped;
    // 现有的方法...
...
public:
    ...
    // void reset();
    // void clear();
    void updateUniformBuffer( std::array<float, 16> &viewMat, std::array<float, 16> &projMat, float time);
    ~GfxQueue();
};