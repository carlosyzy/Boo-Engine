...
public:
    ...
    static void addField(const std::string &className, int type, const std::string &name, const std::string &displayName, ValueType ClassType::*memberPtr)
    {
        PropertyInfo prop;
        prop.type = type;
        prop.name = name;
        prop.displayName = displayName;
        prop.getter = [memberPtr](void *obj) -> json
        {
            ClassType *self = static_cast<ClassType *>(obj);
            return self->*memberPtr;
        };
        prop.setter = [memberPtr](void *obj, const json &j)
        {
            ClassType *self = static_cast<ClassType *>(obj);
            self->*memberPtr = j.get<ValueType>();
        };
